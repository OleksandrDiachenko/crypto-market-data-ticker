...
// domain) - see splash_timer_cb(), which shows this instead of the
// dashboard when gated. Same own-lv_screen approach as the splash, for the
// same dev-screenshot-capturability reason.
...
// Deletes the now-inactive splash screen object, then either shows the
// first-run disclaimer (gated on settings_disclaimer_should_show()) or, if
// already acknowledged for the running firmware version, switches straight
// to the dashboard.
...
// loaded), same reasoning as build_splash_screen(). Built once in
// display_ui_render() regardless of whether it will actually be shown this
// boot - splash_timer_cb() decides that via settings_disclaimer_should_show().
static void build_disclaimer_screen(void)
{
    ...
    lv_label_set_text(intro, "SISWOOD Market Terminal is a free, open-source project provided for "
...
static void display_ui_render(void)
{
    ...
    // switch back to - see build_splash_screen()'s comment. The disclaimer
    // screen is always built too (cheap, and needed the instant
    // splash_timer_cb() decides to show it) even though it may never be
    // shown this boot.
    build_splash_screen();
