...
| P0 | Sparkline handler used `LV_EVENT_DRAW_TASK_ADDED` without `LV_OBJ_FLAG_SEND_DRAW_TASK_EVENTS` | Flag added, then hardware validation reproduced a full LVGL pool exhaustion (`allocate_item: Out of memory`, `LV_ASSERT_MALLOC` -> infinite `while(1)` -> permanent task-WDT freeze) once the flag actually let the callback run against a full 288-point/10-row watchlist, repeating every second via the existing unconditional per-tick chart refresh. Area-fill removed entirely rather than redesigned under time pressure - see `docs/debugging/sparkline-fill-oom-freeze.md`. |
...
| P1 | Physical regression/soak evidence does not yet exist for this audit commit | Full regression (17 screenshot targets, brightness/night mode, empty-watchlist reboot/add-back, 10-symbol REST+WS, Wi-Fi fallback recovery, a real two-cycle OTA install+rollback, 450 navigation cycles) now recorded in `docs/validation/pre-phase-17-release-readiness.md` - and caught the sparkline-fill P0 above. The 60-minute soak was explicitly skipped this pass and remains an open gate; Phase 17 must not start until it runs. |
